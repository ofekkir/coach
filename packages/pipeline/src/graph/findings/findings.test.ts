...
describe('deriveFindings', () => {
  ...
  const { enrichedGraph, findings } = runPipeline(files);

  it('is computable from the enriched graph alone (no pipeline state)', () => {
    expect(deriveFindings(enrichedGraph)).toEqual(findings);
  });

  it('mirrors the graph level and produces at least one session with interactions', () => {
    expect(findings.kind).toBe('agent');
    expect(findings.sessions.length).toBeGreaterThan(0);
    const interactions = findings.sessions.flatMap((s) => s.interactions);
  ...
  it('classifies every interaction and rolls cost/tokens up consistently', () => {
    ...
    const agentCost = findings.rollup.costUsd;
    const summed = findings.sessions.reduce((acc, s) => acc + s.rollup.costUsd, 0);
