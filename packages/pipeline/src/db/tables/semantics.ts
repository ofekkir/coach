...
export const SEMANTICS: TableSpec = {
  ...
  columns: [
    ...
    // prettier-ignore
    { name: 'action', sqlType: 'VARCHAR', doc: "The input-independent action label for this entry, from the ontology vocabulary (e.g. 'load tool schema', 'read source code', 'version control'). The specific input (filename, query, tool name) is NEVER folded in — it lives in repo_path / url. See describe_schema → vocabulary." },
    // prettier-ignore
