...
export const TOOLS: TableSpec = {
  name: 'tools',
  doc: "VIEW (computed on read, never stored) — one row per tool node, projecting the tool columns of `nodes` (WHERE type='tool'). A typed convenience surface; the physical table is still `nodes`, where edges and traversal live.",
