...
// Why: Stage 4 — aggregate every session's canonical nodes into one node table plus
...
// Why: the owning interaction is the root of a node's `parent` chain (interaction nodes
...
// Why: the entity `id` is the value stamped on every node's `sessionId` FK
// (so node.sessionId === Session.id), one Session per distinct harness session id.
