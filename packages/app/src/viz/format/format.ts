 ...
 *  `title` is the verb that leads the card (the first entry's `static`); `subtitle`
 *  the second entry's `static`; `model` the machine id shown at the card's foot. `prompt` is the
 ...
 *  entries supply the verb (entries[0].static) and sub-verb (entries[1].static), with
...
// first entry's static label, the structural tool name backs the tag.
function actionShape(node: ToolNode, semantics: SemanticFields): CardShape {
  return {
    type: 'action',
    tag: toolTag(node.name),
    title: semantics.entries[0]?.static ?? node.name,
    subtitle: semantics.entries[1]?.static,
  };
}
...
function inferenceShape(node: LlmRequestNode, semantics: SemanticFields): CardShape {
  return {
    ...
    title: semantics.entries[0]?.static ?? node.model,
    subtitle: semantics.entries[1]?.static,
    model: node.model,
  };
}
